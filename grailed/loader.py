"""Snowflake loader for the GRAILED schema."""
...
SCHEMA = "GRAILED"
SETUP_SQL_PATH = get_setup_path("grailed")
...
def upload_and_load(conn, query, results_path, images_dir, country="us", language="en"):
    ...
    try:
        ...
        put_json = f"PUT 'file://{results_path}' @SCRAPE_DATA AUTO_COMPRESS=FALSE OVERWRITE=TRUE"
        ...
        cur.execute(f"""
            INSERT INTO PRODUCTS (
                ...
                DESIGNER, CONDITION, SIZE_INFO, SELLER_LOCATION,
                DEPARTMENT, CATEGORY
            )
            SELECT
                {run_id},
                ...
                NULLIF($1:image_path::VARCHAR, ''),
                NULLIF($1:designer::VARCHAR, ''),
                NULLIF($1:condition::VARCHAR, ''),
                NULLIF($1:size_info::VARCHAR, ''),
                NULLIF($1:seller_location::VARCHAR, ''),
                NULLIF($1:department::VARCHAR, ''),
